...
Т.е. вместо хранения индексов 260, 270, 275 листья будут содержать 260, 10, 5 (как попарные дельты с предыдущим значением). <!-- Вот здесь в картинке, похоже, ошибка. Нам же нужно уметь восстанавливать id в первой ветви? --> Это позволяет записать индексы меньшим числом байтов.
