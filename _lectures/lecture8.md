...
Самое общее семейство сетей [[15]], в котором у ребер тоже могут быть признаки (edge features), заданные или обучаемые, которые участвуют с составлении сообщения наряду с представлениями вершин. Это можно записать в виде функции UPDATE:

\\[ \mathbf{h}\_i = \phi \Big( \mathbf{x}\_i, \bigoplus\_{j \in \mathcal{N}(i)} \psi (\mathbf{x}\_i, \mathbf{x}\_j) \Big) \\]

Та же формула, если явно добавить $e\_{ij}$ - edge feature ребра $i,ij$:
