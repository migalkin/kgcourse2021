...
где $\mathbf{W}\_{\lambda(r)} - весовая матрица направлений ребер, которая принимает только три значения: $\mathbf{W}\_{I}$ (incoming) для входящих ребер, $\mathbf{W}\_{O}$ (outgoing) для исходящих, $\mathbf{W}\_{self-loop}$ для петель self-loops.

\\[ \mathbf{W}\_{\lambda(r)} = \begin{cases} \mathbf{W}\_{O} , r \in \mathcal{R} \\  \mathbf{W}\_{I} , r \in \mathcal{R}\_{inv} \\ \mathbf{W}\_{self-loop} , r = \text{self-loop} \end{cases}  \\]
