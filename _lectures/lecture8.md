...
\\[ \mathbf{m}\_{\mathcal{N}(u)} = \sum_{r \in \mathcal{R}} \sum\_{v \in \mathcal{N}\_r(u)} \frac{\alpha\_r \times\_1 \mathbf{B} \times\_2 \mathbf{h}\_v }{f\_n (\mathcal(N)(u), \mathcal{N}(v))} \\]
...
В случае предсказания связей (link prediction) нам нужно оценивать вероятность корректности того или иного триплета $(s,r,o)$, а значит в качестве декодера можно взять любую scoring function из предыдущей лекции, например TransE или DistMult. 
...
Новая архитектура композиционных GCN, compositional GCNs (CompGCN), призвана решить эту и другие проблемы R-GCN.
