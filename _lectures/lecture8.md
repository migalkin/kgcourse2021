...
\\[ \mathbf{z}\_v = \frac{1}{\vert G\_v \vert} \Big( \sum\_{(v,r,u) \in G\_v }  \mathbf{z}\_r \bigodot \mathbf{z}\_u + sum\_{(u,r,v) \in G\_v} \mathbf{z}\_r \bigodot \mathbf{z}\_u \Big)\\]
...
3. Зная максимальную глубину всех подграфов, числовые признаки переводятся в one-hot encoded признаки. Например, при максимальной дистанции 4 hops получается 8-размерный вектор признаков:
```
(0, 1): [1, 0, 0, 0, 0, 1, 0, 0]  
```
